...
if __name__ == '__main__':
    ...
    with tf.device(tf.DeviceSpec(device_type="CPU")):
        ...
        enqueuer = DataFlowToQueue(df, [input_node, heatmap_node, vectmap_node, object_node], queue_size=100)
    ...
    print('split_inp/out', q_inp_split, q_heat_split, q_vect_split, q_obj_split)
    ...
    with tf.device(tf.DeviceSpec(device_type="CPU")):
        ...
        if ',' not in args.lr:
            ...
            learning_rate = tf.train.exponential_decay(starter_learning_rate, global_step, decay_steps=10000, decay_rate=0.33, staircase=True)
    ...
    valid_loss_ll_obj = tf.placeholder(tf.float32, shape=[])
    ...
    with tf.Session(config=config) as sess:
        ...
        while True:
            ...
            if gs_num - last_gs_num >= 100:
                ...
                logger.info('epoch=%.2f step=%d, %0.4f examples/sec lr=%f, loss=%g, loss_ll=%g, loss_ll_paf=%g, loss_ll_heat=%g, loss_ll_obj=%g, q=%d' % (gs_num / step_per_epoch, gs_num, batch_per_sec * args.batchsize, lr_val, train_loss, train_loss_ll, train_loss_ll_paf, train_loss_ll_heat, train_loss_ll_obj, queue_size))
                last_gs_num = gs_num
            ...
            if gs_num - last_gs_num2 >= 1000:
                ...
                average_loss = average_loss_ll = average_loss_ll_paf = average_loss_ll_heat = average_loss_ll_obj = 0
                total_cnt = 0

                if len(validation_cache) == 0:
                    for images_test, heatmaps, vectmaps, objmaps in tqdm(df_valid.get_data()):
                        validation_cache.append((images_test, heatmaps, vectmaps, objmaps))
                    df_valid.reset_state()
                    del df_valid
                    df_valid = None

                # log of test accuracy
                for images_test, heatmaps, vectmaps, objmaps in validation_cache:
                    lss, lss_ll, lss_ll_paf, lss_ll_heat, lss_ll_obj, vectmap_sample, heatmap_sample, objmap_sample = sess.run([total_loss, total_loss_ll, total_loss_ll_paf, total_loss_ll_heat, total_loss_ll_obj, output_vectmap, output_heatmap, output_object], feed_dict={q_inp: images_test, q_vect: vectmaps, q_heat: heatmaps, q_obj: objmaps})
                    ...
                    average_loss_ll_obj += lss_ll_obj * len(images_test)
                    total_cnt += len(images_test)

                logger.info('validation(%d) %s loss=%f, loss_ll=%f, loss_ll_paf=%f, loss_ll_heat=%f, loss_ll_obj=%f' % (total_cnt, training_name, average_loss / total_cnt, average_loss_ll / total_cnt, average_loss_ll_paf / total_cnt, average_loss_ll_heat / total_cnt, average_loss_ll_obj / total_cnt))
                last_gs_num2 = gs_num

                sample_image = [enqueuer.last_dp[0][i] for i in range(4)]
                outputMat = sess.run(outputs, feed_dict={q_inp: np.array((sample_image + val_image)*(args.batchsize // 16))})
                pafMat, heatMat = outputMat[:, :, :, 19:57], outputMat[:, :, :, :19]
                ...
                summary = sess.run(merged_validate_op, feed_dict={
					...
					valid_loss_ll_obj: average_loss_ll_obj / total_cnt,
